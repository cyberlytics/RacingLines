...
io.on("connection", (socket) => {
  socket.on("join_lobby", (data) => {
    ...
    if (clients !== undefined) {
      clients.forEach((client) => {
        roomGameSettings[client] = gameSettings[client];
      });
    }
  ...
  socket.on("gameTempoChanged", (data) => {
    ...
    gameSettings[room.id].GameTempo = data.settingsList[room.id].GameTempo;
    const clients = io.sockets.adapter.rooms.get(data.room);
    const roomGameSettings = {};
    if (clients !== undefined) {
      clients.forEach((client) => {
    ...
    socket.in(data.room).emit("onGameTempoChanged", { roomGameSettings });
  });

  socket.on("canvasSizeChanged", (data) => {
    console.log(data.playerList.CanvasSize);
    players[room.id].CanvasSize = data.settingsList[room.id].CanvasSize;
    const clients = io.sockets.adapter.rooms.get(data.room);
    const joinedPlayers = {};
    if (clients !== undefined) {
      clients.forEach((client) => {
