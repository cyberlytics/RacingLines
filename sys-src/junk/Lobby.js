...
const grid_classes = {
    ...
    useEffect(() => {
        ...
        socket.on('onCanvasSizeChanged', (data) => {
            setSettingsList(data.roomGameSettings);
        });
   ...
   useEffect(() => {
    return  Object.keys(settingsList).map((item,index) => {
        return(
            <option>
                <option value={settingsList[item].CanvasSize} key={index}>
                    {settingsList[item].CanvasSize}
                </option>
                <option value={settingsList[item].GameTempo} key={index}>
                {settingsList[item].GameTempo}
                </option>
            </option>
        )
    })
        ...
        function handleCanvasSizeChanged(event){
            ...
            console.log(newSettingsList[room.id].CanvasSize);
            newSettingsList[room.id].CanvasSize = event.target.value;
            setSettingsList(newSettingsList);
            console.log(newSettingsList[room.id].CanvasSize);
            socket.emit("canvasSizeChanged", {settingsList, room});
        }

        function handleGameTempoChanged(event){
            const newSettingsList = {...settingsList};
            console.log(newSettingsList[room.id].GameTempo);
            newSettingsList[room.id].GameTempo = event.target.value;
            setSettingsList(newSettingsList);
            console.log(newSettingsList[room.id].GameTempo);
            socket.emit("gameTempoChanged", {settingsList, room});
        }
